...
def matrix(ax, x, y, w, h, rows, cols, color, label, sublabel, label_size=30, sublabel_size=20):
...
def dense_delta(ax, x, y, cell, rows, cols, title):
    ...
    ax.text(x + w / 2, y + h + 0.18, title, ha="center", va="bottom", fontsize=27, color=TEXT)
    ax.text(x + w / 2, y - 0.18, r"$\mathrm{rank}(\Delta W_{\mathrm{LoRA}})\leq r$", ha="center", va="top", fontsize=20, color=MUTED)
...
def block_delta(ax, x, y, size, highlight=None):
    ...
    ax.text(x + size / 2, y + size + 0.18, r"$\Delta W_{\mathrm{TiRA}}$", ha="center", va="bottom", fontsize=30, color=TEXT)
...
def delta_wk(ax, x, y, size, shift, highlight_row=1):
    ...
    ax.text(x + size / 2, y + size + 0.14, r"$\Delta W_k$", ha="center", va="bottom", fontsize=24, color=TEXT)
    ax.text(x + size / 2, y - 0.12, r"$\mathrm{rank}(\Delta W_k)=M$", ha="center", va="top", fontsize=18, color=MUTED)
    return highlight_center
...
def tira_block_generator(ax, x, y):
    ...
    matrix(ax, bx, by, bw, bh, 5, 1, BLUE, r"$b_{k,m}$", r"$n_{\mathrm{out}}=d_{\mathrm{out}}/M$", label_size=25, sublabel_size=18)
    ax.text(x + 0.62, y + 0.78, r"$\times$", ha="center", va="center", fontsize=36, color=TEXT)
    matrix(ax, axx, ay, aw, ah, 1, 5, ORANGE, r"$a_{k,m}^{\top}$", r"$n_{\mathrm{in}}=d_{\mathrm{in}}/M$", label_size=25, sublabel_size=18)
    ax.text(x + 2.55, y + 0.78, r"$=$", ha="center", va="center", fontsize=34, color=TEXT)

    rect(ax, cx, cy, cs, cs, fc=GREEN, ec=GREEN, lw=2.3, alpha=0.18)
    grid(ax, cx, cy, cs, cs, 5, 5, color=GRID, lw=0.7)
    ax.text(cx + cs / 2, cy + cs + 0.04, r"$C_{k,m}$", ha="center", va="bottom", fontsize=25, color=TEXT)
    ax.text(
        cx + cs / 2,
        cy - 0.16,
        "place " r"$C_{k,m}$" "\n" r"at block $(m,(m+k)\ \mathrm{mod}\ M)$",
        ha="center",
        va="top",
        fontsize=16,
        color=GREEN,
...
def shifted_group(ax, x, y, size, shift, color, label):
    ...
    ax.text(x + size / 2, y - 0.12, label, ha="center", va="top", fontsize=18, color=MUTED)
...
def build():
    ...
    ax.text(3.05, 6.45, "LoRA", ha="center", va="center", fontsize=42, fontweight="bold", color=TEXT)
    ax.text(11.55, 6.45, "TiRA", ha="center", va="center", fontsize=42, fontweight="bold", color=TEXT)
    ax.text(11.55, 6.05, r"$k$: group index $(0\leq k<K)$; $m$: block-row index $(0\leq m<M)$", ha="center", va="center", fontsize=18, color=MUTED)
    ...
    c_center = tira_block_generator(ax, 7.85, 4.02)
    ...
    ax.text(13.65, 1.30, r"$\mathrm{rank}(\Delta W_{\mathrm{TiRA}})\leq KM$", ha="center", va="top", fontsize=20, color=MUTED)
