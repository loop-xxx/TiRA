...
\begin{abstract}
预训练大语言模型（large language models, LLMs）具备强大的通用语言能力，但在复杂下游任务中仍需要进行任务特定适配。全参数微调通常需要较高的计算与存储成本；相比之下，参数高效微调（parameter-efficient fine-tuning, PEFT）方法通过仅优化少量新增参数显著降低了训练开销。作为代表性方法，LoRA 通过低秩分解实现高效适配，但其适配能力受固定秩 $r$ 的限制，在复杂适配场景中可能出现秩不足问题。针对这一局限，本文提出分块化高秩适配方法 TiRA（Tiled Rank-1 Adaptation）。TiRA 将权重增量矩阵划分为 $M\times M$ 个子块，并利用 $K$ 组短向量外积沿错位块对角线进行结构化填充。在保持与 LoRA 相同可训练参数量的前提下，TiRA 可将有效秩由 $r$ 提升至 $r^2$，从而增强模型对复杂任务的适配能力。TiRA 仍然保持与原始权重可合并的特性，因此在推理阶段不引入额外开销。本文在常识推理、开放域对话生成和数学推理三类任务上对 TiRA 进行了系统评估。实验结果表明，在同等可训练参数预算设置下，TiRA 在 Llama-2-7B 与 Llama-3-8B 等模型上取得优于 LoRA、DoRA、MoRA 和 HiRA 等代表性 PEFT 方法的性能。特别是在 Llama-3-8B 的 GSM8K 数学推理任务上，TiRA 达到 $73.01\%$ 的准确率，较 LoRA 提升 $7.12$ 个百分点，较 HiRA 提升 $2.20$ 个百分点。进一步的分析实验表明，分块粒度 $M$ 对 TiRA 的性能具有重要影响，且较大规模模型通常能够更充分地受益于高秩适配。这些发现验证了结构化分块高秩更新的有效性，为大语言模型在复杂下游任务中的高效适配提供了一种新的技术路径。本文代码与实验相关数据已公开于 https://github.com/loop-xxx/TiRA。
\end{abstract}

\section{引言}

预训练大语言模型具备强大的通用语言能力，但其原始参数通常难以直接适配特定下游任务。全参数微调可以有效提升任务性能，却面临高昂的显存、计算和存储成本。为缓解这一问题，参数高效微调方法通过冻结模型主体参数、仅优化少量新增参数实现任务适配。现有 PEFT 方法大体可分为提示类方法和基于重参数化的方法。提示类方法通过引入可训练软提示或虚拟令牌调控模型行为；以 LoRA\cite{hu2022lora} 为代表的基于重参数化的方法则通过重参数化方式建模网络内部线性层的任务特定权重增量。与提示类方法相比，LoRA 不占用额外上下文长度，且其低秩权重增量可在推理阶段与基础权重合并，因此已成为当前 PEFT 研究中的重要范式。

然而，LoRA 的核心假设是权重增量具有低秩结构。该设计适用于相对简单的任务适配，但在复杂下游任务中，固定秩 $r$ 所张成的低维更新子空间可能不足以刻画任务所需的高维权重变化，从而限制模型的适配能力。直接提高 LoRA 的秩虽然能够提高更新空间维度，但也会同步增加可训练参数量、显存占用和优化成本，削弱 PEFT 方法的效率优势。因此，本文关注如下问题：在维持与 LoRA 相同可训练参数量级、并保留权重可合并特性的前提下，能否构造具有更高有效秩的权重更新形式，以增强大语言模型在复杂下游任务中的适配能力？
...
多步数学推理任务通常要求模型进行较长链条的中间推导\cite{wei2022cot,kojima2022zero}，其复杂度随推理深度显著增加。为进一步验证复杂任务对适配秩的需求，本文在 GSM8K\cite{cobbe2021gsm8k} 训练集上按答案长度构建 Easy 与 Challenge 两个难度子集，答案越长通常对应更长的推理链和更高的推理难度。在此基础上，本文将各子集按 $9:1$ 划分训练集与验证集，并进行秩对比实验，实验超参数见附录~\ref{app:lora_hyperparams}。结果如图~\ref{fig:rank_motivation} 所示。Easy 子集准确率在较低秩区间快速提升，由 $r=8$ 时的 $56.80\%$ 提升至 $r=32$ 时的 $67.47\%$，随后随秩进一步增大出现轻微回落（$r=128$ 时降至 $65.07\%$），呈现早期饱和与高秩冗余特征。相比之下，Challenge 子集在低秩区间提升较为有限，$r=8$、$r=16$ 与 $r=32$ 时均为 $42.90\%$，但在更高秩设置下继续上升，最终在 $r=128$ 时达到 $47.45\%$。该结果说明，任务复杂度越高，模型对更高维更新空间的需求越强，对应所需适配秩也越高；复杂任务对更高适配秩存在实际需求。
...
\begin{enumerate}[leftmargin=2em]
    \item 提出 TiRA，一种面向大语言模型的高秩参数高效适配方法。该方法在严格保持与 LoRA 同等参数预算的前提下，将权重增量矩阵的有效秩由 $r$ 提升至 $r^2$，显著增强模型对复杂下游任务的适配能力与性能。
...
低秩适配（Low-Rank Adaptation, LoRA）是一种经典的基于重参数化的参数高效微调范式\cite{hu2022lora}。该方法冻结预训练主干网络参数，并在线性层中注入一对可训练低秩矩阵作为适配器，通过矩阵乘积 $BA$ 近似权重更新 $\DeltaW$。LoRA 的重要优势在于其与基模型权重具有良好的数学兼容性。微调完成后，低秩旁路参数可被显式折叠为权重增量，并与原始权重矩阵合并为 $W=W_0+\DeltaW$。因此，部署阶段无需保留额外旁路分支，模型前向传播形式与原始线性层保持一致，从而在推理阶段不引入额外计算延迟或显存开销。

在 LoRA 基础上，DoRA\cite{liu2024dora} 从权重分解角度改进低秩适配。该方法将预训练权重显式分解为幅度和方向两个部分，并分别建模二者在微调过程中的变化。其中，幅度分量以可训练参数形式直接更新，方向分量则沿用 LoRA 的低秩更新方式。通过分别调整权重幅度与方向，DoRA 能够在较小参数开销下更接近全参数微调中的权重变化模式；同时，其方向更新仍可在推理前与基础权重合并，因此保留了 LoRA 无额外推理延迟的部署优势。

另一条扩展路线则直接关注 LoRA 低秩更新带来的低维更新空间限制，并在参数高效约束下引入高秩权重更新。MoRA\cite{jiang2024mora} 指出低秩更新可能限制模型学习和记忆新知识的能力，因此采用方阵适配器，并结合非参数的降维与升维算子，在保持可训练参数量相近的同时提升更新矩阵的秩；不过，其静态映射函数也使适配权重与原始权重的直接合并更加复杂。HiRA\cite{huang2025hira} 则利用 Hadamard 积构造高秩适配形式。该方法先通过可训练矩阵 $B$ 和 $A$ 得到低秩矩阵 $BA$，再将其与冻结的预训练权重矩阵 $W_0$ 进行逐元素相乘，即 $\DeltaW = W_0 \odot (BA)$。由于 Hadamard 积满足 $\rank(P\odot Q)\leq \rank(P)\rank(Q)$，当 $W_0$ 本身具有较高秩时，即使 $BA$ 仍是低秩矩阵，最终得到的更新矩阵 $\DeltaW$ 也可能具有更高的秩上界。因此，HiRA 能够在保持与 LoRA 相近可训练参数量的同时提高更新空间维度，并可在推理前将适配权重合并至预训练权重中，从而保留 LoRA 式无额外推理开销的部署特性。

\subsection{其他 PEFT 方法}

PEFT 的另一类重要方法是基于提示的适配方法。该类方法通过在大语言模型的输入层或隐藏层中嵌入可训练虚拟令牌，并仅优化这些提示参数，而保持预训练模型主干参数冻结。代表性工作包括 Prefix Tuning\cite{li2021prefix}、Prompt Tuning\cite{lester2021power} 和 P-Tuning v2\cite{xiao2022ptuning}。其中，Prompt Tuning 在输入序列前端引入软提示以实现任务适配；Prefix Tuning 将可训练前缀注入注意力计算过程；P-Tuning v2 进一步将连续提示扩展至更深层的表示空间，从而增强对复杂任务模式的调控能力。此类方法的性能通常受提示长度、初始化方式和任务形式影响较大。与 LoRA 及其变体直接修改网络内部线性投影不同，提示类方法主要通过调整输入或中间表示空间诱导模型生成任务特定输出。

\section{方法}

\subsection{旁路适配框架}

LoRA 针对神经网络中的线性投影模块实现参数高效微调。鉴于 Transformer\cite{vaswani2017attention} 已成为当前大语言模型的主流架构，其内部的 Query、Key、Value、Output 投影矩阵以及前馈网络的投影层均为线性投影，因此这些模块构成了 LoRA 的主要应用对象。LoRA 冻结预训练网络的全部权重参数，仅在线性投影层旁引入低秩旁路矩阵作为可训练适配器。对于原始权重矩阵 $W\in\R^{d_{\mathrm{out}}\times d_{\mathrm{in}}}$，LoRA 训练一对低秩矩阵 $B\in\R^{d_{\mathrm{out}}\times r}$ 与 $A\in\R^{r\times d_{\mathrm{in}}}$，并以
\begin{equation}
...
\end{equation}
当 $K=M$ 时，$M$ 条错位块对角线刚好覆盖全部 $M\times M$ 个块位置，每个块位置由一个短向量外积生成的秩-1 子块填充。为直观说明错位块对角分组策略，附录~\ref{app:k3m3} 给出了 $K=M=3$ 时 TiRA 增量矩阵的具体构造示例。
...
\end{equation}
当设置 $K=M=r$ 时，TiRA 与 LoRA 的参数量完全一致，均为 $r(d_{\mathrm{out}}+d_{\mathrm{in}})$。二者的关键差异在于秩的扩展能力：每个分组独立贡献的秩上限为 $M$，而 $K$ 个分组叠加后的增量矩阵秩上限可达 $KM$。因此，当 $K=M=r$ 时，TiRA 将有效秩上限由 LoRA 的 $r$ 提升至 $r^2$，从而增强模型在复杂任务中的适配能力。
...
LoRA 的适配能力通常随秩增加而提升，但其参数量也同步线性增长。从数学形式上看，一个秩为 $K\times r$ 的 LoRA 适配模块可表示为 $K$ 个秩为 $r$ 的 LoRA 适配模块的线性叠加。基于这一观察，若为每个秩为 $r$ 的 LoRA 适配模块引入结构化稀疏掩码，使不同模块的更新空间互不重叠且相互补充，则有望在保持原秩-$r$ LoRA 模块参数预算的前提下，将等效秩扩展至原来的 $K$ 倍。

具体地，设 $M=K=r$。第 $k$ 个适配模块产生的增量矩阵可写为 $\Delta W_k=B_kA_k^{\top}$，其中 $B_k\in\R^{d_{\mathrm{out}}\times M}$，$A_k^{\top}\in\R^{M\times d_{\mathrm{in}}}$，且二者均为结构化稀疏矩阵。其稀疏非零模式由 $k$ 控制的循环偏移确定，因此不同适配模块的非零更新位置相互错开并形成互补。这样，单个模块仍保持稀疏结构，而多个模块叠加后能够覆盖更完整的更新位置，从而在保持稀疏参数预算的同时扩展整体更新的表达范围。本文将该结构称为多稀疏 LoRA 专家适配，并在附录~\ref{app:sparse_expert} 给出 $r=3$ 时的具体构造示例。
...
综上，TiRA 继承了多稀疏 LoRA 专家适配的理论可行性与更高维的适配空间，同时其连续分块结构更契合 GPU 内存局部性，可通过高效分块矩阵乘法实现前向传播，避免稀疏索引带来的额外开销。
...
\paragraph{评估指标。}
常识推理任务以准确率作为评价指标。模型基于输入查询生成自由文本回复，随后通过关键词匹配机制从生成内容中提取预测答案。例如，BoolQ 任务识别 ``true'' 或 ``false''，多项选择类任务则匹配各数据集预定义候选答案关键词。对于每个样本，首个匹配到的有效关键词被视为预测结果；若未检测到目标关键词，则判定为错误。该评估流程与 DoRA 和 HiRA 保持一致。开放域对话生成任务采用多种自动生成质量指标进行评估，包括 BLEU\cite{papineni2002bleu}、METEOR\cite{banerjee2005meteor}、ROUGE-L\cite{lin2004rouge} 和 BERTScore\cite{zhang2019bertscore}。数学推理任务遵循领域惯例，以准确率作为评价指标。模型生成的最终答案会与标准答案进行匹配；若最终数值或答案表达与标准答案一致，则判定为正确。具体实现中，本文同时抽取生成文本中的最后一个数值和显式答案提示之后的数值，并分别与标准答案比较；只要其中任一抽取结果与标准答案一致，该样本即判定为正确。

\paragraph{基线方法。}
为全面验证 TiRA 的有效性，本文选取参数高效微调领域的代表性方法作为对比基线，涵盖提示微调与低秩适配两大技术范式。提示微调类方法包括 Prompt Tuning 和 P-Tuning；低秩适配类方法包括 LoRA、DoRA、MoRA 和 HiRA。所有主要实验均在 Llama-2-7B\cite{touvron2023llama2} 与 Llama-3-8B\cite{grattafiori2024llama3} 两款开源大语言模型上完成。除本文复现的 TiRA 结果及分块粒度分析中的 LoRA 对照外，主要基线结果沿用 HiRA 的公开报告，以保证与既有工作的可比性。
...
进一步分析各子任务表现可见，TiRA 在多数推理基准上展现出稳定优势。特别是在 Llama-3-8B 配置下，TiRA 在 SIQA、ARC-c、ARC-e、OBQA、HellaSwag 和 WinoGrande 等多个子任务上达到最优性能。上述结果表明，在同等可训练参数量约束下，TiRA 能够缓解传统低秩适配的低维更新空间限制，为复杂常识推理任务提供更有效的参数微调方案。
...
为进一步从参数空间角度分析 TiRA 权重增量的高秩特征，本文对数学推理实验中训练完成后的适配权重增量 $\DeltaW$ 进行奇异值分解，并与同等参数预算下的 LoRA 进行对比。具体而言，本文选取 Llama-3-8B 在 MetaMath 上训练后的最优检查点，分别构造 LoRA 的 $\DeltaW=BA$ 与 TiRA 的分块化 $\DeltaW$，并在所有适配层和目标线性模块上统计平均谱指标。有效秩采用奇异值阈值 $\tau=0.005$ 计算；能量秩表示累计平方奇异值能量达到给定比例所需的最小秩，即为了覆盖 95\% 或 99\% 的谱能量，需要保留多少个最大的奇异方向。能量秩越高，说明权重更新的能量分布越分散，参与适配的方向越多。稳定秩定义为 $\|\DeltaW\|_F^2/\|\DeltaW\|_2^2$，可视为由谱能量有效支撑的连续型秩指标。若大部分能量集中于最大奇异值对应的主方向，稳定秩较小；若能量分布在更多奇异方向上，稳定秩则相应增大。
...
表~\ref{tab:delta_spectrum} 显示，LoRA 的有效秩严格受限于其设定秩 $r=32$，且其谱能量高度集中，95\% 能量秩仅为 13.55。相比之下，TiRA 在保持与 LoRA（$r=32$）相同可训练参数量的条件下，训练后的 $\DeltaW$ 呈现显著更高的有效秩和更分散的谱能量分布。当 $M=32,K=32$ 时，TiRA 的平均有效秩达到 757.27，95\% 能量秩达到 356.52，分别约为 LoRA 的 23.7 倍和 26.3 倍；稳定秩也由 LoRA 的 2.50 提升至 14.71。该结果说明，TiRA 的结构化分块设计不仅在理论上提高了秩上界，也在实际训练后形成了更高维的权重更新子空间。
...
图~\ref{fig:delta_spectrum_rank95} 进一步展示了不同模块上的谱分布差异。TiRA 在 Query、Key、Value 投影以及前馈网络的上投影和下投影模块上均取得显著高于 LoRA 的 95\% 能量秩，表明高秩更新并非集中于个别层或个别模块，而是较稳定地存在于模型的主要线性投影模块中。其中，下投影和上投影模块的提升尤为明显，说明 TiRA 能够在前馈网络中引入更丰富的任务相关扰动方向。上述定性分析与数学推理性能提升相互印证，说明 TiRA 的性能增益主要来自结构化高秩更新带来的更新空间扩展。

\subsection{分块粒度 $M$ 对适配性能的影响}

当 $M=1$ 时，TiRA 不再进行分块，其结构退化为秩为 $K$ 的低秩分解 $BA$，与 LoRA 形式等价。为分析分块粒度 $M$ 对适配性能的影响，本文固定 $K=32$，使 TiRA 的可训练参数量与 LoRA（$r=32$）严格对齐，并仅改变分块数 $M$。为进一步检验不同模型规模下分块粒度选择的规律，本文选取 Qwen-2-0.5B、Qwen-2-1.5B 和 Qwen-2.5-3B\cite{yang2024qwen2,yang2024qwen25} 三个规模递进的开源大语言模型作为主对照序列，并统一在 MetaMath 上微调 2 个 epoch，在 GSM8K 测试集上评估。详细超参数见附录~\ref{app:ablation_hyperparams}。实验结果如表~\ref{tab:ablation} 和图~\ref{fig:ablation_m_trend} 所示，表中还纳入更大规模的 Llama-3-8B，用于补充观察该规律在更大模型尺度上的外推趋势。

\begin{table}[H]
\centering
\small
\caption{不同规模模型上的分块粒度 $M$ 对比分析。所有配置固定 $K=32$，可训练参数量与同模型上的 LoRA（$r=32$）严格持平。数学评测采用任一答案抽取结果正确即判定正确的规则。最优结果以粗体标示。}
\label{tab:ablation}
\resizebox{\linewidth}{!}{%
...
\paragraph{最优分块粒度随模型规模右移。}
在固定预算 $K=32$ 条件下，最优 $M$ 随模型规模增大而右移，且最优等效秩与隐藏维度之比同步上升。Qwen-2-0.5B、Qwen-2-1.5B 和 Qwen-2.5-3B 的最优 $M$ 分别为 2、8 和 32，对应总秩上限 $KM$ 分别为 64、256 和 1024。与各自隐藏维度（896、1536、2048）相比，最优等效秩占隐藏维度的比例由 $7.1\%$ 增至 $16.7\%$ 和 $50.0\%$。图~\ref{fig:ablation_m_trend} 进一步显示，小模型在较大 $M$ 下会出现性能回落，而较大模型的最优点出现在更高有效秩区间。需要指出的是，由于本组实验固定 $K=32$，且 TiRA 要求 $K\geq M$ 并且 $K$ 为 $M$ 的整数倍，因此可考察的最大分块粒度为 $M=32$。Llama-3-8B 的最佳结果出现在该搜索边界上，说明当前实验范围内较大 $M$ 更有利，但不能排除在更高 $K$ 预算下继续增大 $M$ 仍可能带来收益。这一趋势表明，规模更大的模型更可能承载并受益于更高秩适配。
...
TiRA 的高秩优势并不会随分组数 $M$ 增大而单调增强，而是受到基座模型承载能力的约束。以 Qwen-2-0.5B 为例，当 $M$ 从 2 增大到 4 时准确率已出现下降，继续增大至 $M=8$ 后进一步降至 $36.54\%$，低于 LoRA 基线的 $38.59\%$。Qwen-2-1.5B 也在 $M=16$ 时出现性能回落，低于其最优配置 $M=8$。这说明，当 TiRA 引入的扰动自由度超过模型可有效协调的范围时，额外高秩分量未必带来更强适配能力，反而可能造成性能退化。本文将这一现象称为“秩过载”效应。

较大模型通常具有更高的隐藏维度、更强的参数冗余和更充分的表征空间，任务相关特征更可能分布在相对分离的子空间中，因此多个秩-1 分量可以分别作用于不同低维方向，并通过优化过程组合为全局一致的高秩扰动。相反，小模型的表征空间和可调整自由度有限；当 $M$ 过大时，过多扰动分量可能竞争相同或相近的特征通道，导致梯度方向冲突和优化耦合增强，从而抵消更高维更新空间带来的潜在收益。

上述规律揭示了分块粒度与模型规模之间的耦合关系。随着模型规模增大，表征空间冗余度和可协调的扰动自由度相应提升，模型对高秩适配的承载能力也随之增强。因此，高秩更新是否有效不仅取决于任务是否需要更高适配秩，也取决于基座模型是否具备承载这些高秩扰动的能力。由此可见，小模型上的性能退化不应简单归因于 TiRA 结构失效；更合理的解释是，$M$ 同时调控高秩扰动自由度与分量协同难度，而不同规模模型对这种自由度的承载能力存在差异。因此，TiRA 的最优 $M$ 需要与模型规模及其承载能力相匹配。较大模型更可能从较高 $M$ 中获益，而小模型一旦越过自身承载阈值，则可能进入秩过载区间，导致准确率下降甚至低于 LoRA 基线。

\section{结论}

本文提出 TiRA，一种面向大语言模型的分块化高秩参数高效适配方法。TiRA 在保持与 LoRA 相同可训练参数量级的前提下，通过结构化分块与错位块对角构造，将权重更新的有效秩由 $r$ 扩展至 $r^2$，从而增强模型对复杂推理与生成任务的适配能力。同时，TiRA 保留了 LoRA 可合并、无额外推理开销的优势。大量实验表明，在同等参数预算设置下，TiRA 在常识推理、开放域对话生成和数学推理任务上整体优于现有代表性 PEFT 方法。未来工作将进一步探索 TiRA 在更多模型规模、任务类型和实际应用场景中的表现，并研究分块粒度与模型规模、任务复杂度之间的更一般性匹配规律。
