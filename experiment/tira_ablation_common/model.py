...
from .layer import BaseTiraAblationLayer


def mark_only_tira_ablation_as_trainable(model: nn.Module, bias: str = "none") -> None:
    ...
    if bias == "tira_only":
        for m in model.modules():
            if isinstance(m, BaseTiraAblationLayer) and hasattr(m, "bias") and m.bias is not None:
                m.bias.requires_grad = True
        return
    raise NotImplementedError(f"Unsupported bias type: {bias}")


class BaseTiraAblationModel(torch.nn.Module):
    linear_cls = None
    ...
    def add_adapter(self, adapter_name, config=None):
        ...
        mark_only_tira_ablation_as_trainable(self.model, self.peft_config[adapter_name].bias)
    ...
    def _set_adapter_layers(self, enabled=True):
        for module in self.model.modules():
            if isinstance(module, BaseTiraAblationLayer):
