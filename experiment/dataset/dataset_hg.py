...
class HGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path, split, data_type: TASK_TYPE, training_ratio=1.0):
        self.path = data_path
        load_split = split
        if data_type in [TASK_TYPE.BOOLQ, TASK_TYPE.COMMONSENSE, TASK_TYPE.CONVAI2, TASK_TYPE.METAMATH]:
        ...
        elif data_type == TASK_TYPE.METAMATH:
            input_x = [d['query'] for d in dataset]
            target = [d['response'] for d in dataset]
