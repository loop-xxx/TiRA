...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Analyze the singular-value spectrum of LoRA adapter delta weights. "
            "Use --root_dir to scan all LoRA training runs and write consolidated "
            "results to that root directory, or --checkpoint for a single checkpoint."
        )
    )
    parser.add_argument("--root_dir", type=str, default=None, help="Directory containing LoRA training run folders.")
    ...
    parser.add_argument("--output_prefix", type=str, default="lora_delta_spectrum")
    parser.add_argument(
        "--include_missing",
        action="store_true",
    ...
    parser.add_argument(
        "--no_scale",
        action="store_true",
        help="Do not apply LoRA alpha/r scaling before SVD. Rank-energy metrics are unchanged by scaling.",
    )
...
def is_lora_checkpoint_dir(path: str) -> bool:
    if not (
        os.path.isdir(path)
    ...
    try:
        ...
        return False
    return str(cfg.get("peft_type", "")).lower() == "lora"
...
def immediate_checkpoints(run_dir: str) -> List[str]:
    ...
    for name in os.listdir(run_dir):
        ...
        if is_lora_checkpoint_dir(path):
...
def select_best_checkpoint(run_dir: str) -> Optional[RunCheckpoint]:
    ...
    for state_path, state in trainer_states_for_run(run_dir, checkpoints):
        ...
        if not is_lora_checkpoint_dir(checkpoint):
            checkpoint = os.path.join(run_dir, os.path.basename(os.path.normpath(best_path)))

        if is_lora_checkpoint_dir(checkpoint):
...
def group_lora_weights(state_dict: Dict[str, torch.Tensor]):
    ...
    for key, value in state_dict.items():
        ...
        if key.endswith(".lora_A.weight") or key.endswith(".lora_A"):
            grouped[(layer_id, module_name)]["a"] = value
        elif key.endswith(".lora_B.weight") or key.endswith(".lora_B"):
            grouped[(layer_id, module_name)]["b"] = value
    return grouped


def build_delta_from_lora(a: torch.Tensor, b: torch.Tensor, alpha: Optional[float], apply_scale: bool):
    a = a.to(torch.float32)
    b = b.to(torch.float32)
    r = int(a.shape[0])
    delta = b @ a
    if apply_scale:
        scale = float(r if alpha is None else alpha) / float(r)
        delta = delta * scale
    return delta
...
def analyze_checkpoint(
    ...
    grouped = group_lora_weights(state_dict)

    if not grouped:
        raise ValueError(f"No LoRA layer/module weights were found in {checkpoint_dir}")
    ...
    for layer_id in layer_ids:
        ...
        for module_name in modules:
            ...
            if r_b != r:
                raise ValueError(
                    f"LoRA shape mismatch at layer={layer_id}, module={module_name}: "
                    f"A={tuple(a.shape)}, B={tuple(b.shape)}"
                )

            delta = build_delta_from_lora(a, b, alpha=alpha, apply_scale=apply_scale)
            ...
            row = {
                ...
                "rank_upper_bound": int(min(delta.shape[0], delta.shape[1], r)),
                ...
                "tau": tau,
                "lora_alpha": float(r if alpha is None else alpha),
...
def summarize_rows(detail_rows: Sequence[dict], energy_thresholds: Sequence[float]) -> List[dict]:
    ...
    for (run_name, module_name), rows in sorted(groups.items()):
        ...
        item = {
            ...
            "lora_r": rows[0].get("lora_r", ""),
            "lora_alpha": rows[0].get("lora_alpha", ""),
...
def enrich_rows(rows: List[dict], run: RunCheckpoint, cfg: dict, root_dir: Optional[str]):
...
def main():
    ...
    if args.root_dir is not None:
        ...
        if not runs:
            raise SystemExit(f"No LoRA checkpoint runs found under: {root_dir}")
    else:
        checkpoint = os.path.normpath(args.checkpoint)
        if not is_lora_checkpoint_dir(checkpoint):
            raise SystemExit(f"Not a valid LoRA checkpoint directory: {checkpoint}")
        root_dir = None
