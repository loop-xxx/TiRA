...
class TiraLowerTriangularLayer:
    ...
    def _build_indices(M: int):
        ...
        for offset in range(M):
            for row in range(offset, M):
                offsets.append(offset)
                rows.append(row)
                col = row - offset
                cols.append(col)
                dense_col_idx[offset, row] = col
            for input_col in range(M - offset):
...
class TiraLowerTriangularLinear(nn.Linear, TiraLowerTriangularLayer):
    """TIRA ablation that only writes rank-L blocks on or below the block diagonal."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        TiraLowerTriangularLayer.__init__(self, in_features=in_features, out_features=out_features)
