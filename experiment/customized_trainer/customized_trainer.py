...
class Seq2SeqTrainer(Trainer):
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        """
        Custom PEFT-aware checkpoint loader for resume training.

        HF's default _load_from_checkpoint expects pytorch_model.bin / model.safetensors
        or a sharded index, which PEFT checkpoints don't contain (they only save adapter
        weights). Route PEFT models through load_adapter and fall back to the parent
        implementation for non-PEFT (e.g. FFT).
        """
        if model is None:
            model = self.model

        check_is_custom_peft = any(isinstance(model, _c) for _c in CUSTOM_PEFT_MODEL_NAMES)
        if not check_is_custom_peft:
            return super()._load_from_checkpoint(resume_from_checkpoint, model)

        adapter_bin = os.path.join(resume_from_checkpoint, ADAPTER_WEIGHTS_NAME)
        adapter_safe = os.path.join(resume_from_checkpoint, ADAPTER_SAFE_WEIGHTS_NAME)
        if not (os.path.isfile(adapter_bin) or os.path.isfile(adapter_safe)):
            raise ValueError(
                f"Can't find adapter weights ({ADAPTER_WEIGHTS_NAME} or {ADAPTER_SAFE_WEIGHTS_NAME}) "
                f"in {resume_from_checkpoint}."
            )

        logger.info(f"Loading PEFT adapter weights from {resume_from_checkpoint}")
